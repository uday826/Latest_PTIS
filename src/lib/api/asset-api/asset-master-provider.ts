...
const MASTER_CONFIG: Record<string, {
  ...
  [MASTER_IDS.TYPE]: {
    ...
    fetch: async (p, s, search, g, sortBy, sortOrder) => {
      ...
      return { ...res, items: res.items.map(mapTypeToMasterRecord) };
    },
    getGroups: async () => {
  ...
  [MASTER_IDS.CATEGORY]: {
    ...
    fetch: async (p, s, search) => {
      const res = await getAssetCategoriesPaged(p, s, search);
      return { ...res, items: res.items.map(mapCategoryToMasterRecord) };
    }
  }
};
