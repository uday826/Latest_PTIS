...
import type { FloorDropdownOptions, PoolUnit } from "@/types/asset/floor-details.types";
import {
  ...
  uploadBulkDocumentsAction,
} from "@/app/[locale]/assets/municipal-Asset/add-New-Asset/floor-details/actions";
import { fetchDocumentDefinitionsAction } from "@/app/[locale]/assets/municipal-Asset/add-New-Asset/actions";
...
export function UnitPoolPanel({ dropdownOptions }: UnitPoolPanelProps) {
  ...
  useEffect(() => {
    fetchDepartmentsAction().then((res) => {
  ...
  // ── Load existing sub-units from DB on mount ─────────────────────────────
  useEffect(() => {
    const loadExisting = async () => {
      ...
      try {
        ...
        setParentFloors(parentFloorsList);

        const existing: PoolUnit[] = (res.data as any[]).map((u) => {
          // Resolve floor label from dropdownOptions (floor levels loaded from DB)
          const floorId = u.floorDetailsId ? Number(u.floorDetailsId) : null;
          let floorLabel = "";
          if (floorId) {
            const floorDetail = parentFloorsList.find((f: any) => Number(f.id) === floorId);
            ...
            if (floorLevelId && dropdownOptions?.floorLevels) {
              ...
              const match = dropdownOptions.floorLevels.find(
                (f: any) => String(f.value) === String(floorId)
              );
              floorLabel = match ? `${match.label}` : "Selected";
            } else {
              floorLabel = "Selected";
          ...
          const deptName = deptId
            ? (departments.find((d) => String(d.value) === String(deptId))?.label || u.departmentName || "")
            : (u.departmentName || "");
        ...
        // Only set if pool is empty (don't overwrite if user generated new units)
    ...
    loadExisting();
    // Re-run when dropdownOptions or departments are loaded so labels resolve correctly
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [formData.id, dropdownOptions, departments]);
                ...
                <tbody className="divide-y divide-slate-100">
                  {pool.map((unit) => (
                    <tr key={unit.tempId} className={`hover:bg-slate-50/60 transition-colors ${unit.isSaved && !unit.isModified ? "bg-emerald-50/30" : ""}`}>
                      ...
                      <td className="px-3 py-2 font-semibold text-slate-600 truncate max-w-[120px]" title={(unit.departmentId ? departments.find((d) => String(d.value) === String(unit.departmentId))?.label : "") || unit.departmentName || ""}>
                        {(unit.departmentId ? departments.find((d) => String(d.value) === String(unit.departmentId))?.label : "") || unit.departmentName || "—"}
                      </td>
