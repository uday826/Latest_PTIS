...
export function UnitPoolPanel({ dropdownOptions }: UnitPoolPanelProps) {
  const { formData, registerSubmitHook } = useAssetForm();
    ...
    const loadExisting = async () => {
      ...
      try {
        ...
        const existing: PoolUnit[] = (res.data as any[]).map((u) => {
          ...
          let floorLabel = "";
          if (floorId && dropdownOptions?.floorLevels) {
            const match = dropdownOptions.floorLevels.find(
              (f: any) => String(f.value) === String(floorId)
            );
            floorLabel = match ? `${match.label} Floor` : "Floor Selected";
          }
          ...
          // Unit type: derive from AssetName ("Flat Unit" → "Flat") as fallback
          let unitType = (u.unitType || "").trim();
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="flex items-end">
              <button
                ...
                className={`w-full h-8 rounded-lg text-[10px] font-black uppercase tracking-wider flex items-center justify-center gap-1 transition-colors shadow-sm ${
                  isGenerating || !genType || genCount < 1
          ...
          {pool.length === 0 ? (
            ...
            <div className="rounded-xl border border-slate-200 overflow-hidden">
              <table className="w-full text-xs text-left">
                <thead>
                  <tr className="bg-slate-800 text-slate-200 text-[9px] font-black uppercase tracking-widest">
                ...
                <tbody className="divide-y divide-slate-100">
                  {pool.map((unit) => (
                    <tr key={unit.tempId} className={`hover:bg-slate-50/60 transition-colors ${unit.isSaved && !unit.isModified ? "bg-emerald-50/30" : ""}`}>
                      ...
                      <td className="px-3 py-2 font-semibold text-slate-600 truncate max-w-[120px]" title={unit.departmentName}>
                        {unit.departmentName || "—"}
                      </td>
