...
export function GstMasterTable({
  master,
  selectedGroup,
  onDelete,
  onSave,
  ...
  const columns = useMemo(
    (): Column<MasterDataRecord>[] => [
      {
        key: 'id',
        label: t('table.columns.taxCode').toUpperCase(),
        width: '15%',
        render: (v) => (
      ...
      {
        key: 'name',
        label: t('table.columns.taxName').toUpperCase(),
        width: '25%',
        render: (_, row) => <div className="font-semibold text-sm text-slate-800 break-all whitespace-normal">{row.name}</div>,
      },
      {
        key: 'taxPercentage',
        label: t('table.columns.taxPercentage').toUpperCase(),
        width: '15%',
        render: (v) => (
          <div className="text-sm font-semibold text-slate-700">{v != null ? `${Number(v)}%` : '—'}</div>
        ),
      },
      {
        key: 'effectiveFromDate',
        label: t('table.columns.effectiveFromDate').toUpperCase(),
        width: '15%',
        render: (v) => (
          <div className="text-sm text-slate-600">{v ? String(v).split('T')[0] : '—'}</div>
        ),
      },
      {
        key: 'effectiveToDate',
        label: t('table.columns.effectiveToDate').toUpperCase(),
        width: '15%',
        render: (v) => (
          <div className="text-sm text-slate-600">{v ? String(v).split('T')[0] : '—'}</div>
        ),
