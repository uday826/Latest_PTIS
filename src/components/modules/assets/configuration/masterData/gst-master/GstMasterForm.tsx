...
export function GstMasterForm({
  ...
  const handlePercentageChange = (val: string) => {
    if (val === '') {
      setField('taxPercentage', 0);
      return;
    }
    const num = parseFloat(val);
    if (!isNaN(num)) {
      if (num < 0) setField('taxPercentage', 0);
      else if (num > 100) setField('taxPercentage', 100);
      else setField('taxPercentage', num);
    }
  };
  ...
  return (
    ...
    >
      <div className="px-4 sm:px-6 py-4 sm:py-5 space-y-4 sm:space-y-5">
        ...
        <Input
          label={t('form.labels.taxName')}
          required
          value={formData.name}
          placeholder={t('form.placeholders.taxName')}
        ...
        <Input
          label={t('form.labels.taxPercentage')}
          required
          type="number"
          value={formData.taxPercentage ?? ''}
          placeholder={t('form.placeholders.taxPercentage')}
          ...
          max={100}
          step={0.01}
          error={translateError(errors.taxPercentage)}
          className="placeholder:!text-slate-500 placeholder:!text-[13px] !text-[13px] !text-slate-700"
        />

        <Input
          label={t('form.labels.effectiveFromDate')}
          required
          type="date"
          value={formData.effectiveFromDate ? formData.effectiveFromDate.split('T')[0] : ''}
          onChange={(e) => setField('effectiveFromDate', e.target.value)}
          disabled={isPending}
          fullWidth
          error={translateError(errors.effectiveFromDate)}
          className="placeholder:!text-slate-500 placeholder:!text-[13px] !text-[13px] !text-slate-700"
        />

        <Input
          label={t('form.labels.effectiveToDate')}
          type="date"
          value={formData.effectiveToDate ? formData.effectiveToDate.split('T')[0] : ''}
          onChange={(e) => setField('effectiveToDate', e.target.value || null)}
          disabled={isPending}
          fullWidth
          className="placeholder:!text-slate-500 placeholder:!text-[13px] !text-[13px] !text-slate-700"
        />
