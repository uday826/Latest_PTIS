...
import { Select, type Option } from '@/components/common';
...
interface OwningDepartmentMasterFormProps extends MasterDataFormProps {
  departmentOptions?: Option[];
}

export function OwningDepartmentMasterForm({
  open,
  onClose,
  onSave,
  editData,
  ...
  departmentOptions = [],
  isPending = false,
}: OwningDepartmentMasterFormProps) {
  const t = useTranslations('owningDepartment.configuration.masterData.form');
  const tNames = useTranslations('owningDepartment.masterNames');

  const [departmentId, setDepartmentId] = useState<string>(editData?.departmentId ? String(editData.departmentId) : '');


  const {
    formData, setField, errors, handleSubmit
  } = useMasterDataFormState(masterId, editData, (payload) => {
    const selectedDepartment = departmentOptions.find((option) => String(option.value) === String(departmentId));
    onSave({
      ...payload,
      departmentId: departmentId ? Number(departmentId) : undefined,
      departmentName: selectedDepartment && selectedDepartment.value ? selectedDepartment.label : undefined,
    });
  }, onClose, selectedGroup);

  const isEdit = !!editData;
  const validMasterId = Object.values(MASTER_IDS).includes(masterId as MasterId) ? (masterId as MasterId) : MASTER_IDS.OWNING_DEPARTMENT;
        ...
        <div className="px-4 sm:px-6 py-4 sm:py-5 space-y-4 sm:space-y-5">
          ...
          <div>
            <Select
              ...
              onChange={(_, value) => setDepartmentId(value as string)}
              disabled={isPending}
              className="w-full [&_button]:h-11 [&_button]:rounded-md [&_button]:cursor-pointer"
            />
          </div>
