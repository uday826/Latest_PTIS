...
import { InventoryConditionMasterForm } from './InventoryConditionMasterForm';

export function InventoryConditionMasterTable({
  master,
  selectedGroup,
  onDelete,
  onSave,
  ...
  const columns = useMemo(
    (): Column<MasterDataRecord>[] => [
      ...
      {
        key: 'conditionFactor',
        label: t('configuration.masterData.form.labels.conditionFactor').toUpperCase(),
        width: '15%',
        render: (_, row) => (
          <div className="text-sm text-slate-600 font-medium">{row.conditionFactor ?? '1.0'}</div>
        ),
      },
      {
        key: 'group',
