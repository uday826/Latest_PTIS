...
import { TypeOfUseForm } from './TypeOfUseForm';

export function TypeOfUseTable({
  master,
  selectedGroup,
  onDelete,
  onSave,
  ...
  const columns = useMemo(
    (): Column<MasterDataRecord>[] => [
      {
        key: 'id',
        label: t('table.columns.typeOfUseCode').toUpperCase(),
        width: '25%',
        render: (v) => (
      ...
      },
      {
        ...
        width: '40%',
        render: (_, row) => <div className="font-medium text-sm text-slate-800 break-all whitespace-normal">{row.description}</div>,
      },
      {
        key: 'departmentId',
        label: t('table.columns.groupId').toUpperCase(),
        width: '15%',
        render: (_, row) => <div className="text-sm text-slate-600">{row.departmentId}</div>,
      },
      {
        key: 'status',
