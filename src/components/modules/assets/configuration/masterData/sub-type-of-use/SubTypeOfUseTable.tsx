...
import { SubTypeOfUseForm } from './SubTypeOfUseForm';

export function SubTypeOfUseTable({
  master,
  selectedGroup,
  onDelete,
  onSave,
  ...
  const t = useTranslations('subTypeOfUseMaster');
  ...
  const columns = useMemo(
    (): Column<MasterDataRecord>[] => [
      {
        key: 'description',
        label: t('table.columns.description').toUpperCase(),
        width: '50%',
        render: (_, row) => <div className="font-semibold text-sm text-slate-800 break-all whitespace-normal">{row.description}</div>,
      },
      {
        key: 'displayOrder',
        label: t('table.columns.searchSequence').toUpperCase(),
        width: '20%',
        render: (_, row) => <div className="text-sm text-slate-600">{row.displayOrder}</div>,
      },
      {
        key: 'status',
    ...
    <>
      ...
      <DeleteButton
        onClick={() =>
          confirm({
            variant: 'delete',
            meta: { name: row.description || row.name },
      ...
      <div className="h-full flex flex-col">
        <div className="flex-1 min-h-0" style={{ display: 'flex', flexDirection: 'column' }}>
          <MasterTable
            ...
            tableClassName="min-w-[800px] md:min-w-0"
