...
import { InventoryCategoryMasterForm } from './InventoryCategoryMasterForm';

export function InventoryCategoryMasterTable({
  master,
  selectedGroup,
  onDelete,
  onSave,
  ...
  const columns = useMemo(
    (): Column<MasterDataRecord>[] => [
      ...
      },
      {
        ...
        label: t('configuration.masterData.form.labels.depreciationRate').toUpperCase(),
        width: '15%',
        render: (_, row) => (
          <div className="text-sm text-slate-600 font-medium">{row.depreciationRate ?? '0.1000'}</div>
        ),
      },
      {
        key: 'status',
