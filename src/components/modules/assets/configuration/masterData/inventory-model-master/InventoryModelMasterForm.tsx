...
export function InventoryModelMasterForm({
  open,
  onClose,
  onSave,
  editData,
  ...
  const tErrors = useTranslations('inventoryModel.configuration.masterData.form.errors');
    ...
    <>
      ...
      >
        <div className="px-4 sm:px-6 py-4 sm:py-5 space-y-4 sm:space-y-5">
          ...
          <Select
            label={t('configuration.masterData.form.labels.itemName')}
            required
            options={groupOptions}
            value={formData.group}
            onChange={(_, v) => setField('group', v)}
            placeholder={t('configuration.masterData.form.placeholders.itemName')}
            className="w-full"
            disabled={isPending}
